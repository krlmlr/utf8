...
#' @param width integer giving the minimum field width; specify `NULL` or `NA` for no minimum.
#' @param quote logical scalar indicating whether to surround results with double-quotes and escape internal double-quotes.
#' @param justify justification; one of `"left"`, `"right"`, `"centre"`, or `"none"`.
#'   Can be abbreviated.
#' @param escapes a character string specifying the display style for the backslash escapes,
#'   as an ANSI SGR parameter string, or NULL for no styling.
#' @param display logical scalar indicating whether to optimize the encoding for display, not byte-for-byte data transmission.
#' @param utf8 logical scalar indicating whether to encode for a UTF-8 capable display (ASCII-only otherwise),
#'   or `NULL` to encode for output capabilities as determined by `output_utf8()`.
#' @return A character object with the same attributes as `x` but with `Encoding` set to `"UTF-8"`.
#' @seealso [utf8_print()].
#' @examples
#'
